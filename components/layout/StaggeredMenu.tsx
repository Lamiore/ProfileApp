...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const buildOpenTimeline = useCallback(() => {
    ...
    layerStates.forEach((ls, i) => {
      tl.fromTo(ls.el, { xPercent: ls.start }, { xPercent: 0, duration: 0.5, ease: 'power4.out' }, i * 0.03);
    });
  ...
  const animateColor = useCallback(
    (opening: boolean) => {
      ...
      if (changeMenuColorOnOpen) {
        ...
        colorTweenRef.current = gsap.to(btn, { color: targetColor, duration: 0.25, ease: 'power2.out' });
      } else {
  ...
  return (
    <div
      className={`sm-scope pointer-events-none ${isFixed ? 'fixed top-0 left-0 w-full h-dvh overflow-hidden' : 'w-full h-full'}`}
      style={{ zIndex: 9999 }}
    >
      <div
        className={
                    ...
                    <a
                      ...
                      onClick={(e) => {
                        if (onItemClick) {
                          e.preventDefault();
                          closeMenu();
                          setTimeout(() => onItemClick(it.link), 400);
                        }
                      }}
                    >
