...
const nextIndexRef = { current: null as number | null };

function getNextVariant(): string {
  if (nextIndexRef.current === null) {
    nextIndexRef.current = Math.floor(Math.random() * pathVariants.length);
  } else {
    nextIndexRef.current = (nextIndexRef.current + 1) % pathVariants.length;
  }
  return pathVariants[nextIndexRef.current];
}
...
export function DrawUnderline({ isActive = false, color = "#ffffff", children }: DrawUnderlineProps) {
  ...
  useEffect(() => {
    ...
    if (isActive && !prevActive.current) {
      setCurrentPath(getNextVariant());
    } else if (!isActive && prevActive.current && path) {
      leaveTween.current = gsap.to(path, {
        duration: 0.4,
        drawSVG: "100% 100%",
        ease: "power2.inOut",
        onComplete: () => { setCurrentPath(null); leaveTween.current = null; },
      });
    }
    prevActive.current = isActive;
  }, [isActive]);

  // Animate draw-in whenever currentPath is set
  useEffect(() => {
    const path = pathRef.current;
  ...
  const handleMouseLeave = () => {
    const path = pathRef.current;
    if (!path) { setCurrentPath(null); return; }
