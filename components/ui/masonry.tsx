...
const Masonry: React.FC<MasonryProps> = ({
  ...
  // Preload images and capture natural aspect ratios
  useEffect(() => {
      ...
      items.map(
        (item) =>
          new Promise<{ id: string; ratio: number }>((resolve) => {
            ...
            img.src = item.img;
            img.onload = () =>
              resolve({ id: item.id, ratio: img.naturalHeight / Math.max(img.naturalWidth, 1) });
            img.onerror = () => resolve({ id: item.id, ratio: 1 });
          })
      )
    ).then((results) => {
      const ratios: Record<string, number> = {};
      results.forEach((r) => { ratios[r.id] = r.ratio; });
  ...
  const grid = useMemo<GridItem[]>(() => {
    ...
    return items.map((child) => {
      ...
      const ratio = imageRatios[child.id] ?? 1;
      const height = columnWidth * ratio;
      const y = colHeights[col];
      colHeights[col] += height + gap;
      return { ...child, x, y, w: columnWidth, h: height };
    });
  }, [columns, items, width, imagesReady, imageRatios]);
