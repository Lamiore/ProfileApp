# Windows → Pages Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace floating windows with full pages for About, Blog, Gallery, Connect — keeping a consistent Nav with animated underline on all pages.

**Architecture:** Extract Nav into a standalone component; create an `app/(pages)` route group with a shared layout that renders Nav above the content; move sub-pages into that group. Hero.tsx is simplified to just the grid and LAM badge.
...
| File | Action |
|------|--------|
| `components/Nav.tsx` | **CREATE** — extracted nav with active underline |
| `components/Hero.tsx` | **MODIFY** — remove all window state, use `<Nav />` |
| `components/BlurOverlay.tsx` | **DELETE** — unused after window removal |
| `app/(pages)/layout.tsx` | **CREATE** — shared layout with Nav |
| `app/(pages)/about/page.tsx` | **CREATE** |
| `app/(pages)/blog/page.tsx` | **CREATE** |
| `app/(pages)/gallery/page.tsx` | **CREATE** |
| `app/(pages)/connect/page.tsx` | **CREATE** |
| `app/(pages)/blog/[id]/page.tsx` | **CREATE** (moved from `app/blog/[id]/page.tsx`) |
| `app/blog/[id]/page.tsx` | **DELETE** (replaced by route group version) |
...
git commit -m "feat: add Nav component with active underline"
```

---

### Task 2: Simplify `components/Hero.tsx`

**Files:**
- Modify: `components/Hero.tsx`

Remove all window management state/logic. Replace inline nav JSX with `<Nav />`.
...
git commit -m "refactor: simplify Hero — remove windows, use Nav component"
```

---

### Task 3: Create `app/(pages)/layout.tsx`
...
git commit -m "feat: add (pages) route group layout with Nav"
```

---

### Task 4: Create the four sub-pages

**Files:**
- Create: `app/(pages)/about/page.tsx`
- Create: `app/(pages)/blog/page.tsx`
- Create: `app/(pages)/gallery/page.tsx`
- Create: `app/(pages)/connect/page.tsx`

- [ ] **Step 1: Create `app/(pages)/about/page.tsx`**
...
export default function AboutPage() {
    return (
        <div style={{ maxWidth: "680px", margin: "0 auto", padding: "32px clamp(1rem, 2.6vw, 2.5rem) 64px" }}>
            <AboutWindow />
        </div>
    );
}
...
- [ ] **Step 2: Create `app/(pages)/blog/page.tsx`**
...
export default function BlogPage() {
    return (
        <div style={{ maxWidth: "960px", margin: "0 auto", padding: "32px clamp(1rem, 2.6vw, 2.5rem) 64px" }}>
            <BlogWindow />
        </div>
    );
}
...
- [ ] **Step 3: Create `app/(pages)/gallery/page.tsx`**
...
export default function GalleryPage() {
    return (
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "32px clamp(1rem, 2.6vw, 2.5rem) 64px" }}>
            <GalleryWindow />
        </div>
    );
}
...
- [ ] **Step 4: Create `app/(pages)/connect/page.tsx`**
...
```bash
git add "app/(pages)/about/page.tsx" "app/(pages)/blog/page.tsx" "app/(pages)/gallery/page.tsx" "app/(pages)/connect/page.tsx"
git commit -m "feat: add about, blog, gallery, connect pages"
```

---

### Task 5: Move blog detail page into route group

**Files:**
- Create: `app/(pages)/blog/[id]/page.tsx` (copy from `app/blog/[id]/page.tsx`)
- Delete: `app/blog/[id]/page.tsx` and `app/blog/` directory

- [ ] **Step 1: Create `app/(pages)/blog/[id]/page.tsx`**

Copy the full content of the existing `app/blog/[id]/page.tsx` verbatim — no changes needed.

- [ ] **Step 2: Delete old blog route**
...
```bash
git add "app/(pages)/blog/[id]/page.tsx"
git add -u app/blog
git commit -m "refactor: move blog/[id] into (pages) route group"
```

---

### Task 6: Delete `components/BlurOverlay.tsx`

**Files:**
- Delete: `components/BlurOverlay.tsx`

- [ ] **Step 1: Delete the file**

```bash
rm components/BlurOverlay.tsx
```

- [ ] **Step 2: Verify no remaining imports**

```bash
grep -r "BlurOverlay" app components lib --include="*.tsx" --include="*.ts"
```
...
```bash
git add -u components/BlurOverlay.tsx
git commit -m "chore: delete unused BlurOverlay component"
```
...
Check:
1. Home (`/`) — hero grid renders, nav visible, no underline active, LAM badge present
2. `/about` — About content renders, "About" has animated underline, nav visible
3. `/blog` — Blog listing renders, "Blog" has animated underline
4. `/gallery` — Gallery masonry renders, floating preview window works, "Gallery" has animated underline
5. `/connect` — Connect content + map renders, "Connect" has animated underline
6. `/blog/[id]` — blog detail page still works (test by clicking a blog card)
7. `/card` — still works (untouched)
8. Page transitions — curtain animation fires when navigating between pages
9. Nav on mobile — renders correctly on small viewport
